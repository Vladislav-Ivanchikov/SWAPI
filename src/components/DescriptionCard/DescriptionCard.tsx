...
const DescriptionCard: React.FC = () => {
    ...
    const charDesc = people.filter((item: ICharacter) => item.name === _state.name)
    ...
    return (
        <PeopleDesc>
            {charDesc.map(({name, birth_year, gender, height, mass, eye_color, hair_color, skin_color}: ICharacter) =>
                <CharDesc key={name}>
